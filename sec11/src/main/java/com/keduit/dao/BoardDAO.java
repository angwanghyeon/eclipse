...
public class BoardDAO {
	...
	public void updateBoard(BoardVo bv) {
		...
		try {
			...
			stmt.setString(1, bv.getPass());
			stmt.setString(2, bv.getName());
			stmt.setString(3, bv.getEmail());
			stmt.setString(4, bv.getTitle());
			stmt.setString(5, bv.getContent());
			stmt.setInt(6, bv.getNum());
			stmt.executeUpdate();
			System.out.println("여기까지 잘 왔다."+bv);
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
